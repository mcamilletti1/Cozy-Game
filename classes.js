...
class Monster extends Sprite {
    ...
    attack({attack, recipient, renderedSprites}) {
        ...
        switch (attack.name) {
            case 'Hairball':
                audio.initHairball.play()
                const hairballImage = new Image()
                hairballImage.src = './My Game Assets/spritesheet.png'
                const hairball = new Sprite({
                  position: {
                    x: this.position.x,
                    y: this.position.y
                  },
                ...
                renderedSprites.splice(1, 0, hairball)

                gsap.to(hairball.position, {
                    x: recipient.position.x,
                    y: recipient.position.y,
                    onComplete: () => {
                        audio.hairballHit.play()
                        gsap.to(healthBar, {
